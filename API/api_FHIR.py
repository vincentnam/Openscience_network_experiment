...
def operator_list_def(operator):
    with open("operator_list_sql.dict","rb") as fp:
        return pickle.load(fp)[operator]
...
def request_metadata():
    ...
    -H "initiator:1" -H "platform-id:1" -H "jump:1" -H "platforms-visited:''" -H "operator:lower or equal" -H "operand:90" -H "request-id:ll9"
...
def get_node_nearest_from_distribution():
    ...
    for index in range(1,len(degree_distrib:=registry["network"]["degrees_distribution"])):
        degree = index+2
...
def get_node_to_link_to():
    ...
    if "existing-model" not in request.headers:
    ...
    registry["platforms"][app.config["PLATFORM-ID"]].append(platform_toadd_id)
    save_registry()
    return get_node_nearest_from_distribution()
...
# curl -X GET 193.168.1.10:5000/inscription -H 'platform-id:new-platform' -F {"matchs":{"new-platform-1:{}"}}
# curl -X GET 193.168.1.10:5000/inscription -H 'platform-id:new-platform' -d {matchs:{new-platform-1:1}}